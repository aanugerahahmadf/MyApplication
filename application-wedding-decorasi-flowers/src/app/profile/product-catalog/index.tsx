...
export default function ProductCatalogScreen() {
  ...
  const renderItem = ({ item }: { item: any }) => {
    ...
    return (
      <TouchableOpacity
        onPress={() => router.push({ pathname: '/details', params: { id: item.id, type: 'product' } })}
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.backgroundElement }]}>
      <Stack.Screen options={{ title: 'Flower Catalog', headerShown: true }} />
      {loading && !refreshing ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <Text style={{ textAlign: 'center', marginTop: 40, color: colors.textSecondary }}>No products found</Text>
          }
        />
      )}
    </View>
